...
$catquery = "SELECT iVehicleCategoryId,vHomepageLogo,vCategory_" . $_SESSION['sess_lang'] . " as vehicalcategory FROM `vehicle_category` WHERE iParentId = 0 and eStatus = 'Active' order by iDisplayOrder";
$vcatdata = $obj->MySQLSelect($catquery);

$default_lang = $_SESSION['sess_lang'];	 
if ($default_lang == "" || $default_lang == NULL){
	$lmquery = "SELECT vCode FROM `language_master` WHERE  eDefault= 'Yes'";
	$datalm = $obj->MySQLSelect($lmquery);
	$default_lang = $datalm[0]['vCode'];
} 
			
$hmquery = "SELECT third_mid_title_two1 FROM `homecontent` WHERE vCode = '". $default_lang ."'";
$datahm = $obj->MySQLSelect($hmquery);
...
<div id="main-uber-page">
    ...
    <div class="our-services">
        <div class="our-services-inner">
            <h2><?php echo $datahm[0]['third_mid_title_two1']; ?></h2>
            <ul>
                <?php
                if (!empty($vcatdata)) {
