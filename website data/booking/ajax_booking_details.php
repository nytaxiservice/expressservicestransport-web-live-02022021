...
if ($type == 'getVehicles') {
    if ($eType == "UberX") {
        ...
        $sql23 = "SELECT vt.*,lm.vLocationName FROM `vehicle_type` AS vt LEFT JOIN `country` AS c ON c.iCountryId=vt.iCountryId left join location_master as lm ON lm.iLocationId = vt.iLocationid WHERE (lm.iCountryId='" . $countryid . "' OR vt.iLocationid = '-1') AND vt.eType='" . $eType . "' AND vt.eStatus = 'Active' AND ePoolStatus = 'No' $locations_where ORDER BY vt.iVehicleTypeId ASC";
    }
