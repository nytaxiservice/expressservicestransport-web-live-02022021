	...
	for($i=0;$i< count($data_ex_doc);$i++){
		...
		if($data_ex_doc[$i]['doc_usertype'] == "company"){
			...
			if(!empty($data_ex_doc[$i]['Cmpname'])){
				$cmplink = $tconfig["tsite_url"].''.SITE_ADMIN_URL.'/company_document_action.php?id='.$data_ex_doc[$i]['cmpid'].'&action=edit';
		...
		if($data_ex_doc[$i]['doc_usertype'] == "driver"){
			...
			$link = $tconfig["tsite_url"].''.SITE_ADMIN_URL.'/driver_document_action.php?id='.$data_ex_doc[$i]['driverid'].'&action=edit&user_type=driver';
