...
function getTripFare($Fare_data, $surgePrice) {
    global $generalobj, $obj, $ENABLE_WAITING_CHARGE_RENTAL, $ENABLE_WAITING_CHARGE_FLAT_TRIP, $SERVICE_PROVIDER_FLOW;
    //$ALLOW_SERVICE_PROVIDER_AMOUNT = $generalobj->getConfigurations("configurations", "ALLOW_SERVICE_PROVIDER_AMOUNT");
    $tripData = get_value('trips', 'vCouponCode,iUserId,eType,iVehicleTypeId,iDriverVehicleId,eServiceLocation,tVehicleTypeData, tVehicleTypeFareData', 'iTripId', $Fare_data[0]['iTripId'], '', '');
    ...
    $categoryData = get_value('vehicle_category', 'iParentId,ePriceType,eMaterialCommision, fCommision', 'iVehicleCategoryId', $iVehicleCategoryId, '', '');
    ...
    if ($SERVICE_PROVIDER_FLOW == "Provider" && $eType == "UberX") {
        ...
        $categoryData = get_value('vehicle_category', 'ePriceType,eMaterialCommision, fCommision', 'iVehicleCategoryId', $iParentId, '', '');
        $ePriceType = $categoryData[0]['ePriceType'];
        $eMaterialCommision = $categoryData[0]['eMaterialCommision'];
        $fMaterialCommisionPer = $categoryData[0]['fCommision'];
    }
...
function calculateFare($totalTimeInMinutes_trip, $tripDistance, $vehicleTypeID, $iUserId, $priceRatio, $startDate = "", $endDate = "", $couponCode = "", $tripId, $fMaterialFee = 0, $fMiscFee = 0, $fDriverDiscount = 0, $waitingTime = 0, $totalHoldTimeInMinutes_trip = 0, $personData = array()) {
    global $generalobj, $obj, $ENABLE_WAITING_CHARGE_RENTAL, $ENABLE_WAITING_CHARGE_FLAT_TRIP, $ENABLE_INTRANSIT_SHOPPING_SYSTEM, $ENABLE_AIRPORT_SURCHARGE_SECTION, $HOTEL_BOOKING_SERVICE_CHARGE, $SERVICE_PROVIDER_FLOW, $SYSTEM_PAYMENT_FLOW, $data_trips, $POOL_ENABLE;
    ...
    /* changes for rental */
    $sql = "select iDriverId,fPickUpPrice,fNightPrice,iQty,eFareType,eFlatTrip,fFlatTripPrice,fVisitFee,fTollPrice,eTollSkipped,fWaitingFees,eHailTrip,iRentalPackageId,fOutStandingAmount,eWalletDebitAllow,eBookingFrom,iHotelBookingId,eType,ePaymentBy,iOrganizationId,iHotelId,fAirportPickupSurge,fAirportDropoffSurge,tVehicleTypeData, tUserWalletBalance,ePoolRide from trips where iTripId='" . $tripId . "'";
    ...
    if ($ENABLE_AIRPORT_SURCHARGE_SECTION == 'Yes') {
        ...
        $total_fare = $total_fare + $fAirportPickupSurgeAmount;
        $fTripGenerateFare = $fTripGenerateFare + $fAirportPickupSurgeAmount;
        /* Extra Airport Pickup surge */
        /* Extra Airport Pickup surge */
        ...
        $total_fare = $total_fare + $fAirportDropoffSurgeAmount;
        $fTripGenerateFare = $fTripGenerateFare + $fAirportDropoffSurgeAmount;
        /* Extra Airport Pickup surge */
    }
    ...
    if (strtoupper(PACKAGE_TYPE) == "SHARK") {
        if ($totalHoldTimeInMinutes_trip > 0 && $ENABLE_INTRANSIT_SHOPPING_SYSTEM == "Yes") {
            ...
            $total_fare = $total_fare + $fTripHoldPrice;
            $fTripGenerateFare = $fTripGenerateFare + $fTripHoldPrice;
            $fTripGenerateFare_For_Commission = $fTripGenerateFare_For_Commission + $fTripHoldPrice;
        }
    }
    ...
    if ($eBookingFrom == 'Kiosk') {
        ...
        if ($HOTEL_BOOKING_SERVICE_CHARGE > 0) {
            ...
            $fTripGenerateFare = $fTripGenerateFare + $Commision_Fare_Hotel;
            $total_fare = $total_fare + $Commision_Fare_Hotel;
        }
    }
    ...
    if ($eWalletDebitAllow == "Yes") {
        ...
        if ($user_available_balance > 0) {
            $totalCurrentActiveTripsCount = getCurrentActiveTripsTotal($iUserId);
            ...
            if ($totalCurrentActiveTripsCount > 1 && ($SYSTEM_PAYMENT_FLOW == "Method-2" || $SYSTEM_PAYMENT_FLOW == 'Method-3')) {
            ...
            if (strtoupper(PACKAGE_TYPE) == "SHARK") {
                ...
                /*
                    Solved By webster for Bug 
                    http://192.168.1.141/bug-system/view.php?id=3292
                    11Feb2020
                    $total_fare is above $walleetFare i just bring it down
                */
                $total_fare = 0;
            }
...
function calculateFareEstimateAll($totalTimeInMinutes_trip, $tripDistance, $vehicleTypeID, $iUserId, $priceRatio, $startDate = "", $endDate = "", $couponCode = "", $surgePrice = 1, $fMaterialFee = 0, $fMiscFee = 0, $fDriverDiscount = 0, $DisplySingleVehicleFare = "", $eUserType = "Passenger", $iQty = 1, $SelectedCarTypeID = "", $isDestinationAdded = "Yes", $eFlatTrip = "No", $fFlatTripPrice = 0, $sourceLocationArr = array(), $destinationLocationArr = array(), $DisplayMultiDeliveryFare = "", $RideType = "Ride", $scheduleDate = "") {
    ...
    if ($DisplySingleVehicleFare == "") {
        ...
        if ($Fare_data[0]['eFareType'] == "Regular") {
            ...
            $Fare_data[0]['total_fare_amount'] = $generalobj->setTwoDecimalPoint($total_fare);
        } else {
            $Fare_data[0]['total_fare_amount'] = $generalobj->setTwoDecimalPoint($Fare_data[0]['FinalFare']);
        }
        ...
        if ($eFlatTrip == "Yes") {
            ...
            if ($currData[0]['eRoundingOffEnable'] == "Yes") {
                $tripFareDetailsArr[$i + 1]['total_fare_amount'] = $generalobj->setTwoDecimalPoint($roundingOffTotal_fare_amount);
            } else {
                $tripFareDetailsArr[$i + 1]['total_fare_amount'] = $generalobj->setTwoDecimalPoint($Fare_data[0]['total_fare_amount']);
            }
            //$tripFareDetailsArr[$i + 1]['total_fare_amount'] = $generalobj->setTwoDecimalPoint($Fare_data[0]['total_fare_amount']);
...
function getTripPriceDetails($iTripId, $iMemberId, $eUserType = "Passenger", $PAGE_MODE = "HISTORY") {
    global $obj, $generalobj, $tconfig, $DELIVERY_VERIFICATION_METHOD, $POOL_ENABLE, $ENABLE_INTRANSIT_SHOPPING_SYSTEM, $SERVICE_PROVIDER_FLOW, $APP_TYPE, $ENABLE_FAVORITE_DRIVER_MODULE, $parent_ufx_catid;
    $returnArr = array();
    //$iTripId = 1582;
    ...
    if ($SERVICE_PROVIDER_FLOW == "Provider" && $tripData[0]['eType'] == "UberX" && !empty($tripData[0]['tVehicleTypeFareData'])) {
        ...
        if ($ParentVehicleCategoryId == 0) {
            ...
            if (count($tVehicleTypeFareDataArr_fareArr) > 0) {
                ...
                $sql_parent_id = "SELECT (SELECT vcs.vCategory_" . $userlangcode . " FROM vehicle_category as vcs WHERE vcs.iVehicleCategoryId = vc.iParentId) as vCategory FROM vehicle_category as vc, vehicle_type as vt WHERE vc.iVehicleCategoryId = vt.iVehicleCategoryId AND vt.iVehicleTypeId = '" . $tVehicleTypeFareDataArr_fareArr[0]->id . "'";

                $parent_data_arr = $obj->MySQLSelect($sql_parent_id);
                $tripData[0]['vCategory'] = $parent_data_arr[0]['vCategory'];
            }
        } else {
            $tripData[0]['vCategory'] = get_value("vehicle_category", "vCategory_" . $userlangcode, "iVehicleCategoryId", $ParentVehicleCategoryId, '', 'true');
        }
        ...
        if (!empty($parent_ufx_catid) && $parent_ufx_catid > 0) {
            $sql_parent_id = "SELECT vc.vCategory_" . $userlangcode . " as vCategory FROM vehicle_category as vc, vehicle_type as vt WHERE vc.iVehicleCategoryId = vt.iVehicleCategoryId AND vt.iVehicleTypeId = '" . $tripData[0]['iVehicleTypeId'] . "'";
        } else {
            $sql_parent_id = "SELECT (SELECT vcs.vCategory_" . $userlangcode . " FROM vehicle_category as vcs WHERE vcs.iVehicleCategoryId = vc.iParentId) as vCategory FROM vehicle_category as vc, vehicle_type as vt WHERE vc.iVehicleCategoryId = vt.iVehicleCategoryId AND vt.iVehicleTypeId = '" . $tripData[0]['iVehicleTypeId'] . "'";
        }
        $parent_data_arr = $obj->MySQLSelect($sql_parent_id);
    ...
    $TripDeliveryData = array();
    if ($eType == "Multi-Delivery") {
        $returnArr['ePaymentByReceiverForDelivery'] = "No";
        $sqldeliverydata = "SELECT * FROM `trips_delivery_locations` WHERE ( iActive='Active' OR iActive='On Going Trip')  AND iTripId='" . $iTripId . "' ORDER BY  iTripDeliveryLocationId ASC LIMIT 0,1";
        $TripDeliveryData = $obj->MySQLSelect($sqldeliverydata);
        if (count($TripDeliveryData) > 0) {
            // $sql="select vValue from trip_delivery_fields where iDeliveryFieldId='2' and iTripDeliveryLocationId ='".$TripDeliveryData[0]['iTripDeliveryLocationId']."'";
            ...
            $Data_Trip_Update['tEndLat'] = $TripDeliveryData[0]['tEndLat'];
            $Data_Trip_Update['tEndLong'] = $TripDeliveryData[0]['tEndLong'];
            $Data_Trip_Update['tDaddress'] = $TripDeliveryData[0]['tDaddress'];
            $Data_Trip_Update['vDeliveryConfirmCode'] = $TripDeliveryData[0]['vDeliveryConfirmCode'];
            $Data_Trip_Update['vReceiverName'] = $vReceiverName;
            $Data_Trip_Update['vReceiverMobile'] = "+" . $vPhoneCode . $vReceiverMobile;
            //Added By HJ On 08-01-2019 For Removed Country Code As Per Discuss With DT Start
            if ($eType == "Multi-Delivery") {
                $Data_Trip_Update['vReceiverMobile'] = $vReceiverMobile;
            }
            ...
            $sql = "SELECT count(iTripDeliveryLocationId) AS TotalTripDelivery FROM trips_delivery_locations WHERE iTripId='" . $iTripId . "'";
            $TotalTripDeliveryData = $obj->MySQLSelect($sql);
            $returnArr['TotalTripDeliveryData'] = $TotalTripDeliveryData[0]['TotalTripDelivery'];
        }
        ...
        for ($j = 0; $j < count($Data_tripLocations); $j++) {
            if ($Data_tripLocations[0]['ePaymentBy'] == "Sender") {
                ...
                if ($Data_tripLocations[$j]['ePaymentByReceiver'] == "Yes") {
                    $ReceNo = $j + 1;
                    $sql = "select vValue from trip_delivery_fields where iTripId ='$iTripId' and iDeliveryFieldId = '2' and iTripDeliveryLocationId = '" . $Data_tripLocations[$j]['iTripDeliveryLocationId'] . "'";
                    $db_trip_fields = $obj->MySQLSelect($sql);
                    $returnArr['PaymentPerson'] = $languageLabelsArr['LBL_RECEIPENT_TXT'] . $ReceNo . " (" . $db_trip_fields[0]['vValue'] . ")";
                    break;
                }
            }
        }
    ...
    if (count($TripDeliveryData) > 0) {
        ...
        $returnArr['Running_Receipent_Detail'] = $languageLabelsArr['LBL_RECEIPENT_TXT'] . " " . $iRunningTripDeliveryNo . " ( " . $vReceiverName . " )";
    }
    ...
    if ($eType == "Multi-Delivery") {
        ...
        if (($vTripStatus == "Active" && $vDriverTripStatus == "Arrived" && $eSignVerification == "No") || ($tripData[0]['ePaymentCollect_Delivery'] == "No" && $vDriverTripStatus == "Arrived")) {
            ...
            $IS_OPEN_FOR_SENDER = "Yes";
        }
        if ($IS_OPEN_SIGN_VERIFY == "No") {
            $sqldelivdata = "SELECT eSignVerification FROM `trips_delivery_locations` WHERE ( iActive='Canceled' OR iActive='Finished')  AND eSignVerification = 'No' AND iTripId='" . $iTripId . "' ORDER BY iTripDeliveryLocationId ASC LIMIT 0,1";
        ...
        $returnArr['IS_OPEN_SIGN_VERIFY'] = "No";
        $returnArr['IS_OPEN_FOR_SENDER'] = "No";
    ...
    if ($eUserType == "Passenger") {
        ...
        if ($PAGE_MODE != 'Display') {
            $iFare = $tripData[0]['fTripGenerateFare'] + $tripData[0]['fTipPrice'] - $fCommision - $tripData[0]['fTax1'] - $tripData[0]['fTax2'] - $tripData[0]['fOutStandingAmount'] - $tripData[0]['fHotelCommision'];
        } else {
    ...
    if ($eUserType == "Passenger") {
        $sqlp = "SELECT cu.vName, cu.iCurrencyId, cu.eRoundingOffEnable,cu.ratio FROM register_user AS ru LEFT JOIN currency AS cu ON ru.vCurrencyPassenger = cu.vName WHERE ru.iUserId = '" . $iMemberId . "'";
        $currData = $obj->MySQLSelect($sqlp);
        $vCurrency = $currData[0]['vName'];
    ...
    if ($eUserType == "Passenger") {
        ...
        if ($eFlatTrip == "Yes" && $iActive != "Canceled") {
            ...
            if ($extraPersonCharge > 0) {
                $tripFareDetailsArr[][$languageLabelsArr['LBL_EXTRA_PERSON_CHARGE']] = ($iActive != "Canceled") ? $currencySymbol . $extraPersonCharge : "--";
                $i++;
            }
          ...
          $tripFareDetailsArr[0][$languageLabelsArr['LBL_Total_Fare']] = $currencySymbol." 0.00";
            ...
            if ($fSurgePriceDiff > 0) {
                ...
                $i++;
                $normalfare = $fTripGenerateFare - $fSurgePriceDiff - $fTax1 - $fTax2 - $fMinFareDiff - $fWaitingFees - $fOutStandingAmount - $fHotelCommision - $fAirportPickupSurgeAmount - $fAirportDropoffSurgeAmount - $fTripHoldPrice - $fMaterialFee - $fMiscFee + $fDriverDiscount; // $fTripHoldPrice Variable For In Transite Amount By HJ On 28-12-2018
                if ($eTollSkipped == "No") {
                    $normalfare = $fTripGenerateFare - $fSurgePriceDiff - $fTax1 - $fTax2 - $fMinFareDiff - $fWaitingFees - $fOutStandingAmount - $fTollPrice - $fHotelCommision - $fAirportPickupSurgeAmount - $fAirportDropoffSurgeAmount - $fTripHoldPrice; // $fTripHoldPrice Variable For In Transite Amount By HJ On 28-12-2018
                }
                $normalfare = formatNum($normalfare);
            ...
            if ($fSurgePriceDiff == 0 && ($fAirportPickupSurgeAmount > 0 || $fAirportDropoffSurgeAmount > 0)) {
                $tripFareDetailsArr[]['eDisplaySeperator'] = "Yes";
                $i++;
                $normalfare = $fTripGenerateFare - $fSurgePriceDiff - $fTax1 - $fTax2 - $fMinFareDiff - $fWaitingFees - $fOutStandingAmount - $fHotelCommision - $fAirportPickupSurgeAmount - $fAirportDropoffSurgeAmount - $fTripHoldPrice;
                if ($eTollSkipped == "No") {
                    $normalfare = $fTripGenerateFare - $fSurgePriceDiff - $fTax1 - $fTax2 - $fMinFareDiff - $fWaitingFees - $fOutStandingAmount - $fTollPrice - $fHotelCommision - $fAirportPickupSurgeAmount - $fAirportDropoffSurgeAmount - $fTripHoldPrice;
                }
                $normalfare = formatNum($normalfare);
                $tripFareDetailsArr[][$languageLabelsArr['LBL_NORMAL_FARE']] = ($iActive != "Canceled") ? $currencySymbol . $normalfare : "--";
                $i++;
...
function getTripWaitingFee($iTripId) {
    global $generalobj, $obj, $ENABLE_WAITING_CHARGE_RENTAL, $ENABLE_WAITING_CHARGE_FLAT_TRIP, $SERVICE_PROVIDER_FLOW;
    $sql = "SELECT tStartDate,tDriverArrivedDate,iVehicleTypeId,vTripPaymentMode,eHailTrip,iWaitingFeeTimeLimit,eType,iRentalPackageId,eFlatTrip, eFareType, tVehicleTypeData, tVehicleTypeFareData FROM trips WHERE iTripId='" . $iTripId . "'";
    $tripdata = $obj->MySQLSelect($sql);
...
function UpdateTripOutstandingAmount($iTripId, $ePaidByPassenger = "No", $ePaidToDriver = "No") {
    global $generalobj, $obj;
    $sql = "SELECT iUserId,iDriverId,fCancellationFare,fWalletDebit,vTripPaymentMode,vRideNo,iVehicleTypeId, tVehicleTypeFareData FROM trips WHERE iTripId='" . $iTripId . "'";
    $tripdata = $obj->MySQLSelect($sql);
    $iUserId = $tripdata[0]['iUserId'];
    $iDriverId = $tripdata[0]['iDriverId'];
...
function AppTypeFilterArr($iMemberId, $UserType, $vLang) {
