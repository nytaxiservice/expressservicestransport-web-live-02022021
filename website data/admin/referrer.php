...
$getUserRefData = $obj->MySQLSelect("SELECT CONCAT(ru.vName,' ',ru.vLastName) AS passangerName,CONCAT(rd1.vName,' ',rd1.vLastName) AS OrgdriverName,ru.eRefType as eRefType , ru.iUserId,ru.iRefUserId FROM " . $refTableName . " as ru LEFT JOIN " . $tableName . " as rd1 on rd1.$primaryId=ru.iRefUserId WHERE ru.eRefType = '" . $userType . "'");
