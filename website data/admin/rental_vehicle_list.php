...
if ($Vehicle_type_name == "Ride-Delivery") {
    ...
    if ($APP_TYPE == 'UberX') {
        $sql = "SELECT count(vt.iVehicleTypeId) as Total,vc.iVehicleCategoryId,vc.vCategory_" . $default_lang . " from  vehicle_type as vt left join vehicle_category as vc on vt.iVehicleCategoryId = vc.iVehicleCategoryId where vt.eType='" . $Vehicle_type_name . "' AND vt.ePoolStatus =  'No' AND vt.estatus ='Active' $ssql";
    } else {
...
if ($Vehicle_type_name == "Ride-Delivery") {
    ...
    left join location_master as lm ON lm.iLocationId = vt.iLocationid 
		where vt.eType='" . $Vehicle_type_name . "' AND vt.ePoolStatus='No' $ssql $adm_ssql $ord LIMIT $start, $per_page";
    } else if ($APP_TYPE == 'Ride-Delivery-UberX') {
    ...
    left join location_master as lm ON lm.iLocationId = vt.iLocationid 
		...
		left join city as ct ON ct.iCityId = vt.iCityId 
    left join location_master as lm ON lm.iLocationId = vt.iLocationid 
		where eType='" . $Vehicle_type_name . "' AND vt.ePoolStatus='No' $ssql $adm_ssql $ord LIMIT $start, $per_page";
    }
}
