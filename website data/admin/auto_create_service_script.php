...
define("VEHICLE_CATEGORY", "vehicle_category");
...
$getCategoryData = $obj->MySQLSelect("SELECT vCategory_EN,iVehicleCategoryId,iParentId,vLogo,vLogo1,ePriceType,eBeforeUpload,eAfterUpload,eShowType,eMaterialCommision,vBannerImage,eCatType,eSubCatType,eFor,eDeliveryType,iServiceId,tBannerButtonText,eDetailPageView,fCommision,fWaitingFees,iWaitingFeeTimeLimit,fCancellationFare,iCancellationTimeLimit FROM vehicle_category WHERE iParentId='0' AND eStatus != 'Deleted' AND eCatType='ServiceProvider'");
$getSubCategoryData = $obj->MySQLSelect("SELECT vCategory_EN,iVehicleCategoryId,iParentId,vLogo FROM vehicle_category WHERE iParentId > 0 AND eStatus != 'Deleted' AND eCatType='ServiceProvider'");
...
for ($c = 0; $c < count($getCategoryData); $c++) {
    ...
    if (count($catArr) > 0) {
        ...
        for ($s = 0; $s <= count($categoryCount); $s++) {
            ...
            $cateId = $obj->MySQLQueryPerform("vehicle_category", $descArr, 'insert');
