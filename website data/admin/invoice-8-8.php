...
if(empty($latitudes[0]) && strtoupper($ENABLE_STOPOVER_POINT)=='YES') {
    $latitudes = array();
    $longitudes = array();
    $sql = "select tActualDestLatitude,tActualDestLongitude from trips_stopoverpoint_location where iTripId = '" . $iTripId . "'";
    //$sql = "select tDestLatitude,tDestLongitude from trips_stopoverpoint_location where iTripId = '" . $iTripId . "'";
    $data_locations = $obj->MySQLSelect($sql);
    foreach($data_locations as $key=>$value) { 
        //$latitudes[] = $value['tDestLatitude'];
        $latitudes[] = $value['tActualDestLatitude'];
        $longitudes[] = $value['tActualDestLongitude'];
...
<?php if ($db_trip_data['eType'] == 'Deliver') { ?>
                                                    ...
                                                    <table style="width:100%" cellpadding="5" cellspacing="0" border="0">
                                                        ...
                                                        <?php if(!empty($db_trip_data['vDeliveryConfirmCode'])) { ?>
                                                        <tr>
