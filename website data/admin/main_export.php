...
if ($section == 'rider') {
    ...
    $ssql1 = "AND (vEmail != '' OR vPhone != '')";
    $sql = "SELECT CONCAT(vName,' ',vLastName) as `User Name`,vEmail as Email,CONCAT(vPhoneCode,' ',vPhone) AS Mobile,iUserId AS `Wallet Balance`,eStatus as Status FROM register_user WHERE 1=1 $eStatus_sql $ssql $ssql1 $rdr_ssql $ord";
    // filename for download
    if ($type == 'XLS') {
        $filename = $timestamp_filename . ".xls";
        header("Content-Disposition: attachment; filename=\"$filename\"");
...
if ($parent_ufx_catid > 0) {
    $getSubCat = $obj->MySQLSelect("SELECT GROUP_CONCAT(DISTINCT CONCAT('''',iVehicleCategoryId, '''')) SUB_CAT FROM vehicle_category WHERE iParentId='" . $parent_ufx_catid . "'");
...
if ($section == 'coupon') {
    ...
    $sql = "SELECT vCouponCode as `Gift Certificate`,fDiscount as `Discount`,eValidityType as `ValidityType`,
            ...
            END AS `ExpiryDate`,
            iUsageLimit as `Usage Limit`,iUsed as `Used`,eStatus as `Status`$field FROM coupon WHERE 1 $ssql $ord";
...
if ($section == 'sub_service_category') {
    ...
    if ($parent_ufx_catid != "0") {
        ...
        $sql = "SELECT vCategory_" . $default_lang . " as SubCategory, (SELECT vCategory_" . $default_lang . " FROM vehicle_category WHERE iVehicleCategoryId='" . $sub_cid . "') as Category, (select count(iVehicleTypeId) from vehicle_type where vehicle_type.iVehicleCategoryId = vehicle_category.iVehicleCategoryId) as `Service Types`, iDisplayOrder as `Display Order`, eStatus as Status FROM vehicle_category WHERE (iParentId='" . $sub_cid . "' || iVehicleCategoryId='".$parent_ufx_catid."') AND  1 = 1 $ssql $ord";
    } else {
        $sql = "SELECT vCategory_" . $default_lang . " as SubCategory, (SELECT vCategory_" . $default_lang . " FROM vehicle_category WHERE iVehicleCategoryId='" . $sub_cid . "') as Category,(select count(iVehicleTypeId) from vehicle_type where vehicle_type.iVehicleCategoryId = vehicle_category.iVehicleCategoryId) as `Service Types`, iDisplayOrder as `Display Order`,eStatus as Status FROM vehicle_category WHERE (iParentId='" . $sub_cid . "' || iVehicleCategoryId='".$parent_ufx_catid."') $ssql $ord";
    }
