...
//Pagination End
if (!empty($eStatus)) {
    $sql = "SELECT vc.iVehicleCategoryId,vc.vBannerImage, vc.vLogo,vc.vCategory_" . $default_lang . " as vCategory, vc.eStatus, vc.iDisplayOrder,vc.eCatType, (select count(iVehicleCategoryId) from vehicle_category where iParentId = vc.iVehicleCategoryId AND eStatus != 'Deleted') as SubCategories FROM vehicle_category as vc WHERE  vc.iParentId='0' $ssql $rdr_ssql $ord LIMIT $start, $per_page";
} else {
