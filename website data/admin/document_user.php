...
if (isset($_POST['submit'])) {
    ...
    if($id != "") {
		$msg= $generalobj->checkDuplicateAdmin('iAdminId', 'administrators' , Array('vEmail'),$tconfig["tsite_url"]."/".SITE_ADMIN_URL."/admin_action.php?success=3&var_msg=Email already Exists", "Email already Exists",$id ,"");
	}else {
		$msg= $generalobj->checkDuplicateAdmin('vEmail', 'administrators' , Array('vEmail'),$tconfig["tsite_url"]."/".SITE_ADMIN_URL."/admin_action.php?success=3&var_msg=Email already Exists", "Email already Exists","" ,"");
	}
     if($msg == 1){
        if($id == ""){
