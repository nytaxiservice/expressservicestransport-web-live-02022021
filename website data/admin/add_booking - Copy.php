...
//print_r($WHEEL_CHAIR_ACCESSIBILITY_OPTION);die;
$script = "booking";
...
<?php } ?>
                                ...
                                <div class="total-price total-price1" > <b>Fare Estimation</b>
                                    <input type="hidden" name="dist_fare" id="dist_fare_hidden">
                                    <input type="hidden" name="time_fare" id="time_fare_hidden">
                                    <hr>
                                    <ul>
                                        <li id="MinFare">
                                            <b>Minimum Fare</b> :
                                                    ...
                                                    if (($("#from").val() != "" && $("#from_lat_long").val() != "") && ($("#to").val() != "" && $("#to_lat_long").val() != "")) {
                                                        ...
                                                        directionsService.route(request, function (response, status) {
                                                            if (status == google.maps.DirectionsStatus.OK) {
                                                                ...
                                                                dist_fare_new = dist_fare.toFixed(2);
                                                                $('#dist_fare_hidden').val(dist_fare_new);
                                                                var time_fare = parseFloat($("#duration").val(), 10) / parseFloat(60, 10);
                                                                $('#time_fare').text(time_fare.toFixed(2));
                                                                time_fare_new = time_fare.toFixed(2);
                                                                $('#time_fare_hidden').val(time_fare_new);
                                                                ...
                                                                $.ajax({
                                                                    ...
                                                                    success: function (dataHtml)
                                                                    {
                                                                        if (dataHtml != "") {
                                                                            ...
                                                                            $('#dist_fare_price').text(parseFloat(fPricePerKM * dist_fare_new).toFixed(2));
                                                                            /* var eunit = $("#eUnit").val();
                                                                             if(eunit == "Miles"){
                                                                             $('#dist_fare_price').text(parseFloat((fPricePerKM*($('#dist_fare').text()*1.6))).toFixed(2));
                                                                             } */
                                                                            $('#time_fare_price').text(parseFloat(fPricePerMin * time_fare_new).toFixed(2));
                                                        ...
                                                        $.ajax({
                                                            ...
                                                            success: function (dataHtml)
                                                            {
                                                                if (dataHtml != "") {
                                                                    ...
                                                                    $('#minimum_fare_price').text(iMinFare);
                                                                    $('#base_fare_price').text(iBaseFare);
                                                                    $('#dist_fare_price').text(parseFloat(fPricePerKM * $('#dist_fare_hidden').val()).toFixed(2));
                                                                    /* var eunit = $("#eUnit").val();
                                                                     if(eunit == "Miles"){
                                                                     $('#dist_fare_price').text(parseFloat((fPricePerKM*($('#dist_fare').text()*1.6))).toFixed(2));
                                                                     } */

                                                                    $('#time_fare_price').text(parseFloat(fPricePerMin * $('#time_fare_hidden').val()).toFixed(2));
                                                                    if (ENABLE_TOLL_COST == 'Yes') {
                                                                        if ($('#fTollPrice').val() > 0 && $('#eTollSkipped').val() == 'No' && eFlatTrip != 'Yes' && eType != 'UberX') {
                                                                            $("#toll_price").show();
                                                                            $('#toll_price_val').text(tollcostval);
    ...
    <div class="modal-dialog modal-large">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>- Admin will need to communicate & confirm with <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> and then select him as <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> so the Ride can be allotted to him. </p>
                ...
                <p>- Both <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> and Rider will receive the booking details through Email and SMS as soon as the form is submitted. Based on this booking details, <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> will pickup the rider at the scheduled time.</p>
                <p>- They both will get the reminder SMS and Email as well before 30 minutes of actual trip</p>
                <p>- The assigned <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> can see the upcoming Bookings from his App under "My Bookings" section.</p>
                <p>- <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> will have option to "Start Trip" when he reaches the Pickup Location at scheduled time or "Cancel Trip" if he cannot take the ride for some reason. If the <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> clicks on "Cancel Trip", a notification will be sent to Administrator so he can make alternate arrangements.</p>
                <p>- Upon clicking on "Start Trip", the ride will start in <?php echo strtolower($langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']); ?>'s App in regular way.</p>
                <p>&nbsp;</p>
                <p><b>Auto Assign <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> </b>: The "Ride Later" booking made from the mobile application has the "<?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> Auto Assign" by default enabled. Furthermore Admin can also select this option while adding the booking manually from admin panel</p>
                <p>- <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> auto assignment process works as explained below</p>
                <p>- System will automatically sends the request to <?php echo strtolower($langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN']); ?> who are online and available within pickup location radius</p>
                <p>- <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?>(s) will get the 30 seconds dial screen request  before 8-12 minutes before the actual pickup time. This request is same like "Request Now" one.</p>
                <p>- If no <?php echo strtolower($langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']); ?>(s) accepts the request then system will make a 2nd try after 4 minutes and sends the request again . At this point system also notifies the admin through email that no any <?php echo strtolower($langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN']); ?> had accepted the request in first try.</p>
                <p>- Again If no <?php echo strtolower($langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']); ?>(s) accepts the request then system will make a 3rd and last try after 4 minutes and send the request again. At this point system also notifies admin through email that no any <?php echo strtolower($langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN']); ?> had accepted the request in 2nd try.</p>
                <p>- System makes the 3 trials and if in any trial, no <?php echo strtolower($langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN']); ?> available in that area then it will inform administrator about unavailability of <?php echo strtolower($langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']); ?> so administrator takes necessary action to contact that rider and arrange the taxi for him</p>
...
<div class="modal fade" id="myModalufx" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-large">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p><b>Flow </b>: Through "Manual Booking" Feature, you can book <?php echo $langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN']; ?> for users who ordered for a Service by calling you. There will be users who may not have iPhone or Android Phone or may not have app installed on their phone. In this case, they will call Company (your company) and order service which may be needed immediately or after some time later.</p>
                ...
                <p>- Admin will need to communicate & confirm with <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> and then select him as <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> so the Job can be allotted to him. </p>
                ...
                <p>- Both <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> and User will receive the booking details through Email and SMS as soon as the form is submitted. Based on this booking details, <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> will go to user's location at the scheduled time.</p>
                <p>- They both will get the reminder SMS and Email as well before 30 minutes of actual job</p>
                <p>- The assigned <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> can see the upcoming Bookings from his App under "My Jobs" section.</p>
                <p>- <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> will have option to "Start Job" when he reaches the Job Location at scheduled time or "Cancel Job" if he cannot take the job for some reason. If the <?php echo strtolower($langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']); ?> clicks on "Cancel Job", a notification will be sent to Administrator so he can make alternate arrangements.</p>
                <p>- Upon clicking on "Start Job", the service  will start in <?php echo strtolower($langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']); ?>'s App in regular way.</p>
                <p>&nbsp;</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
