...
if (ONLYDELIVERALL == 'Yes') {
    $sql = "SELECT rd.iDriverId,rd.vEmail,rd.tRegistrationDate,rd.vPhone,rd.vCode,rd.eStatus,rd.eIsFeatured,(SELECT count(dv.iDriverVehicleId) FROM driver_vehicle AS dv WHERE dv.iDriverId=rd.iDriverId AND dv.eStatus != 'Deleted' AND dv.iMakeId != 0 AND dv.iModelId != 0 AND dv.eType != 'UberX') AS `count`,CONCAT(rd.vName,' ',rd.vLastName) AS driverName FROM register_driver rd WHERE 1=1 $eQuery $ssql $ssql1 $dri_ssql $ord LIMIT $start, $per_page";
} else {
    $sql = "SELECT rd.iDriverId,rd.vEmail,rd.tRegistrationDate,rd.vPhone,rd.vCode,rd.eStatus,rd.eIsFeatured,(SELECT count(dv.iDriverVehicleId) FROM driver_vehicle AS dv WHERE dv.iDriverId=rd.iDriverId AND dv.eStatus != 'Deleted' AND dv.iMakeId != 0 AND dv.iModelId != 0 AND dv.eType != 'UberX') AS `count`,CONCAT(rd.vName,' ',rd.vLastName) AS driverName, c.vCompany,c.eStatus as cmp_status FROM register_driver rd LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId WHERE 1=1 $eQuery $ssql $ssql1 $dri_ssql $ord LIMIT $start, $per_page";
}
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th width="18%"><a href="javascript:void(0);" onClick="Redirect(3,<?php
                                                        ...
                                                        <th width="12%" class="align-center"><a href="javascript:void(0);" onClick="Redirect(6,<?php if ($sortby == '6') {
                                                                echo $order;
                                                            } else { ?>0<?php } ?>)">Taxi Count <?php if ($sortby == 6) {
                                                    ...
                                                    <th width="12%" class="align-center"><a href="javascript:void(0);" onClick="Redirect(5,<?php
                                                            ...
                                                            if ($sortby == 5) {
                                                                ...
                                                                ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                        <? if (ONLYDELIVERALL == 'No' && ($APP_TYPE == 'UberX' || $APP_TYPE == 'Ride-Delivery-UberX')) { ?>
