...
//$temp = $generalobj->get_benefit_amount(925);
...
if(empty($latitudes[0]) && strtoupper($ENABLE_STOPOVER_POINT)=='YES') {
	...
	$sql = "select tStartLat,tStartLong,tEndLat,tEndLong from trips where iTripId = '" . $iTripId . "'";    $data_locations = $obj->MySQLSelect($sql);   
    ...
    //$sql = "select tDestLatitude,tDestLongitude from trips_stopoverpoint_location where iTripId = '" . $iTripId . "'";
    $data_locations_mul = $obj->MySQLSelect($sql);
	foreach($data_locations_mul as $key=>$value) { 
        //$latitudes[] = $value['tDestLatitude'];
        ...
        //$longitudes[] = $value['tDestLongitude'];
    }
...
if(empty($latitudes[0]) || empty($data_locations_mul)) { 
    ...
    $longitudes = array();  
	$sql = "select tStartLat,tStartLong,tEndLat,tEndLong from trips where iTripId = '" . $iTripId . "'";    $data_locations = $obj->MySQLSelect($sql);   
	$lat_array[0] = $data_locations[0]['tStartLat'];    $lat_array[1] = $data_locations[0]['tEndLat'];    $long_array[0] = $data_locations[0]['tStartLong'];    $long_array[1] = $data_locations[0]['tEndLong'];
	$total_ele = count($lat_array);
	$inc = 1;
	if ($total_ele > 200) {
		$inc = round($total_ele / 200);
                                                ...
                                                if ($db_trip_data['eType'] == 'Ride') {
                                                    if (isset($strpOverPointData) && !empty($strpOverPointData)) {
                                                        foreach ($strpOverPointData as $strpOverPointDatakey => $strpOverPointDatavalue) {
                                                            ...
                                                            ?> 
                                                            <span class="location-to"><i class="icon-map-marker"></i> <b><?= @date('h:i A', @strtotime($strpOverPointDatavalue['tReachedTime'])); ?><p><?php if(!empty($strpOverPointDatavalue['tActualDAddress'])) echo $strpOverPointDatavalue['tActualDAddress']; else echo $strpOverPointDatavalue['tDAddress']; ?></p></b></span>
