...
if (ONLYDELIVERALL == 'Yes') {
    $sql = "SELECT dv.iDriverVehicleId, dv.iDriverId, dv.eStatus, m.vMake, md.vTitle,CONCAT(rd.vName,' ',rd.vLastName) AS driverName, dv.eType FROM driver_vehicle dv, register_driver rd, make m, model md WHERE 1=1 AND dv.iDriverId = rd.iDriverId  AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId $eQuery $ssql $dri_ssql $ord LIMIT $start, $per_page";
} else {
    if ($APP_TYPE == 'UberX') {
        $sql = "SELECT dv.iDriverVehicleId,dv.eStatus,CONCAT(rd.vName,' ',rd.vLastName) AS driverName,dv.vLicencePlate, c.vCompany, dv.eType FROM driver_vehicle dv, register_driver rd,company c WHERE 1=1 AND dv.iDriverId = rd.iDriverId  AND dv.iCompanyId = c.iCompanyId $eQuery $ssql $dri_ssql";
    } else {
        $sql = "SELECT dv.iDriverVehicleId, dv.iDriverId, dv.eStatus, m.vMake, md.vTitle,CONCAT(rd.vName,' ',rd.vLastName) AS driverName, c.vCompany, dv.eType FROM driver_vehicle dv, register_driver rd, make m, model md, company c WHERE 1=1 AND dv.iDriverId = rd.iDriverId AND dv.iCompanyId = c.iCompanyId AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId $eQuery $ssql $dri_ssql $ord LIMIT $start, $per_page";
    }
}
...
<?php } ?>
    ...
    echo $order;
} else { ?>0<?php } ?>)"><?php echo $langage_lbl_admin['LBL_VEHICLE_CAPITAL_TXT_ADMIN']; ?> <?php if ($sortby == 1) {
                                                ...
                                                <? if (ONLYDELIVERALL != 'Yes') { ?>
                                                    ...
                                                    echo $order;
                                                } else { ?>0<?php } ?>)">Company <?php if ($sortby == 2) {
                                                    if ($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php }
                                        } else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                <? } ?>
