...
if (!empty($_SERVER['HTTP_HOST']) && $_SERVER['HTTP_HOST'] == "192.168.1.141" && !empty($_SERVER['REQUEST_URI']) && (strpos($_SERVER['REQUEST_URI'], 'cubejekshark') !== false) == true && isset($_REQUEST['type']) && !empty($_REQUEST['type'])) {
...
if ($type != "generalConfigData" && $type != "signIn" && $type != "isUserExist" && $type != "signup" && $type != "LoginWithFB" && $type != "sendVerificationSMS" && $type != "countryList" && $type != "changelanguagelabel" && $type != "requestResetPassword" && $type != "UpdateLanguageLabelsValue" && $type != "staticPage" && $type != "sendContactQuery" && $type != "generateReviewModeLogin" && $type != "signup_kiosk_passanger" && $type != "getAdvertisementBanners" && $type != "insertBannereImpressionCount" && $type != "getNewsNotification") {
    ...
    if ($tSessionId == "" || $GeneralMemberId == "" || $GeneralUserType == "") {
        ...
        if ($GeneralUserType == "Hotel" || strtolower($GeneralUserType) == "kiosk") {
...
if ($appVersion != "") {
    ...
    } else if ($UserType == "Hotel" || strtolower($UserType) == "kiosk") {
...
function getPassengerDetailInfo($passengerID, $cityName = "", $LiveTripId = "") {
    ...
    global $generalobj, $obj, $demo_site_msg, $PHOTO_UPLOAD_SERVICE_ENABLE, $parent_ufx_catid, $generalConfigArr, $tconfig, $vTimeZone, $vUserDeviceCountry, $_REQUEST, $intervalmins, $generalConfigPaymentArr, $ENABLE_RIDER_ADVERTISEMENT_BANNER, $ADVERTISEMENT_TYPE;
    ...
    if (count($row) > 0) {
        ...
        $row[0]['vFlutterwaveCurrency'] = get_value('currency', 'vName', 'eDefault', 'Yes', '', 'true');

        $vFlutterwavevSymbol = get_value('currency', 'vSymbol', 'eDefault', 'Yes', '', 'true');
        ...
        $db_label_flutter = $obj->MySQLSelect($sql);

        $LBL_FLUTTERWAVE_CHARGE_MSG_TXT = $db_label_flutter[0]['vValue'];

        $FLUTTERWAVE_CHARGE_AMOUNT = $row[0]['FLUTTERWAVE_CHARGE_AMOUNT'];
        ...
        if ($TripStatus != "NONE") {
            ...
            $sql = "SELECT iTripId,eUserType FROM `ratings_user_driver` WHERE iTripId='$TripID' AND eUserType='Passenger'";
        ...
        $sql = "SELECT ord.iOrderId,ord.iCompanyId,ord.iDriverId,ord.vOrderNo FROM `orders` as ord WHERE ord.iUserId='" . $passengerID . "' AND ord.iStatusCode = '6' AND (select count(iRatingId) from ratings_user_driver as rud where rud.iOrderId=ord.iOrderId AND rud.eFromUserType = 'Passenger' ) = 0  ORDER BY ord.iOrderId DESC LIMIT 0,1";
        ...
        $getDataCategory = $obj->MySQLSelect("SELECT iVehicleCategoryId FROM vehicle_category WHERE `eCatType` =  'MoreDelivery' AND  `eFor` =  'DeliveryCategory'");

        $row[0]['DELIVERY_CATEGORY_ID'] = $getDataCategory[0]['iVehicleCategoryId'];



        $getCatName = $obj->MySQLSelect("SELECT vCategory_" . $vLanguage . " FROM vehicle_category WHERE iVehicleCategoryId='" . $getDataCategory[0]['iVehicleCategoryId'] . "'");
...
function getDriverDetailInfo($driverId, $fromSignIN = 0) {
    ...
    global $generalobj, $obj, $demo_site_msg, $PHOTO_UPLOAD_SERVICE_ENABLE, $parent_ufx_catid, $generalConfigArr, $vTimeZone, $tconfig, $vUserDeviceCountry, $intervalmins, $generalConfigPaymentArr, $POOL_ENABLE, $ENABLE_DRIVER_ADVERTISEMENT_BANNER, $ADVERTISEMENT_TYPE, $APP_TYPE, $_REQUEST, $MAX_DRIVER_DESTINATIONS;
    ...
    if (count($Data) > 0) {
        ...
        $Data[0]['vFlutterwaveCurrency'] = get_value('currency', 'vName', 'eDefault', 'Yes', '', 'true');
        ...
        $getGeneralPaymentArray = $generalobj->getGeneralVarAll_Payment_Array();

        $FLUTTERWAVE_CHARGE_AMOUNT = $getGeneralPaymentArray['FLUTTERWAVE_CHARGE_AMOUNT'];
        ...
        $FLUTTERWAVE_CHARGE_AMOUNT = $Data[0]['FLUTTERWAVE_CHARGE_AMOUNT'];
        ...
        if ($TripStatus != "NONE") {
            ...
            if ($Data[0]['eSystem'] == "DeliverAll") {
                ...
                $sql = "SELECT iTripId,eUserType FROM `ratings_user_driver` WHERE iTripId='$TripID' AND eUserType='Driver'";
            ...
            if (strtoupper(APP_TYPE) == "UBERX" || strtoupper(APP_TYPE) == "RIDE-DELIVERY-UBERX") {
                ...
                //echo "<pre>";print_R($UberX_Trip_Charge);die;
...
if ($type == "signIn") {
    ...
    if ($UserType == "Passenger") {
        ...
        if (count($Data) > 0) {
            ...
            if ($Data[0]['eStatus'] == "Active") {
                ...
                if ($Data[0]['eStatus'] != "Deleted") {
                    ...
                    $returnArr['iUserId'] = $Data[0]['iUserId'];
        ...
        $sql = "SELECT h.iHotelId,a.eStatus,h.vLang,h.vImgName,a.vPassword FROM `hotel` as h LEFT JOIN administrators as a on a.iAdminId = h.iAdminId WHERE a.vEmail='$Emid' OR a.vContactNo = '$Emid'";
...
if ($type == "getDetail") {
    ...
    if ($UserType == "Passenger") {
        ...
        $sql = "SELECT iGcmRegId,vTripStatus,vLang,eChangeLang,iTripId FROM `register_user` WHERE iUserId='$iUserId'";
        ...
        $sql = "SELECT iGcmRegId,vLang,eChangeLang,iTripId FROM `register_driver` WHERE iDriverId='$iUserId'";
        ...
        if (count($Data) > 0) {
            ...
            if ($currencttripid > 0) {
                ...
                $TripData = get_value("trips", 'iServiceId,eSystem', 'iTripId', $Data[0]['iTripId']);
...
if ($type == "loadAvailableCab") {
    ...
    if ($_REQUEST['GeneralUserType'] == 'Hotel' || strtolower($_REQUEST['GeneralUserType']) == "kiosk") {
    ...
    if ($APP_TYPE == "Ride-Delivery-UberX" || $APP_TYPE == "Ride-Delivery" || $APP_TYPE == "Delivery") {
        ...
        if ($eShowOnlyMoto == "Yes") {
            ...
            if ($eRental == "No") {
                ...
                //if ($CheckRideDelivery['eMotoRideEnable'] == "No" || ($eRental == "Yes" && $CheckRideDelivery['eMotoRentalEnable'] == "No") || $CheckRideDelivery['eMotoDeliveryEnable'] == "No") {
...
if ($type == "CheckPromoCode") {
    ...
    $eType = isset($_REQUEST['eType']) ? clean($_REQUEST['eType']) : ''; //'Ride','Delivery','UberX','DeliverAll','Deliver'
    ...
    if ($eType == "" || $eType == NULL) {
        ...
        if ($eType == "Deliver" || $eType == "deliver") {

            $eType = "Delivery";

        }

        $ssql .= " AND (eSystemType = '" . $eType . "' OR eSystemType = 'General')";

    }



    $sql = "SELECT * FROM coupon where eStatus = 'Active' AND vCouponCode = '" . $promoCode . "' $ssql ORDER BY iCouponId ASC LIMIT 0,1";

    $data = $obj->MySQLSelect($sql);

    if (count($data) > 0) {

        //$sql = "select iTripId from trips where vCouponCode = '$promoCode' and iActive = 'Finished' and iUserId='$iUserId'"; //Comment By HJ As Per Discuss With KS On 31-12-2018

        $sql = "select iTripId from trips where vCouponCode = '$promoCode' and iUserId='$iUserId'";
        ...
        $booking_data = $obj->MySQLSelect("select vCouponCode from `cab_booking` where `vCouponCode`='" . $promoCode . "' and `iUserId`='$iUserId' and `eStatus`!='Declined' and `eStatus`!='Cancel' and `eStatus`!='Completed'");

        //Added By HJ On 04-09-2019 For Check Later Booking Promocode Used Or Not End

        if (!empty($data_coupon) || count($booking_data) > 0) {

            $returnArr['Action'] = "01"; // code is already used one time

            $returnArr["message"] = "LBL_COUPON_CODE_ALREADY_USED";
            ...
            if ($iUsageLimit <= $iUsed) {
                ...
                $returnArr["message"] = "LBL_COUPONCODE_COMPLETE_USAGE_LIMIT";
            ...
            if ($eValidityType == "Permanent") {
                ...
                if ($dActiveDate <= $curr_date && $dExpiryDate >= $curr_date) {
                    ...
                    $returnArr["message"] = "LBL_COUPON_CODE_EXPIRED";
...
if ($type == 'getEstimateFareDetailsArr') {
    ...
    $total_fare_amount = str_replace(",", "", $Fare_data[count($Fare_data) - 1]['total_fare_amount']);
...
if ($type == "submitRating") {
    ...
    if (count($row_check) > 0) {
        ...
        if ($isCollectTip == "Yes" && $userType == "Passenger" && $eType == "Ride") {
...
if ($type == "getCabRequestAddress") {
    ...
    $fields = "iCabRequestId,iVehicleTypeId,eType,tSourceAddress,tDestAddress,tUserComment,iRentalPackageId,ePayType,ePayWallet,fDistance,fTripGenerateFare,fDuration,iFare,fDiscount,fWalletDebit,eServiceLocation,tVehicleTypeData, iHotelBookingId";
    ...
    $tVehicleTypeData = (array) json_decode($Data_cab_request[0]['tVehicleTypeData']);
    ...
    if (count($tVehicleTypeData) > 0) {
        ...
        $getMainCat = $obj->MySQLSelect("SELECT VC.vCategory_" . $vLang . " AS vVehicleCategory,VT.iVehicleCategoryId,if(VC.iParentId >0,(SELECT vCategory_" . $vLang . " FROM vehicle_category VC1 WHERE VC.iParentId=VC1.iVehicleCategoryId),'') AS vVehicleCategory FROM vehicle_type VT INNER JOIN vehicle_category VC ON VT.iVehicleCategoryId=VC.iVehicleCategoryId WHERE iVehicleTypeId='" . $tVehicleTypeData[0]->iVehicleTypeId . "'");
...
if ($type == "GetMemberWalletBalance") {
    ...
    $returnArr['user_available_balance'] = strval($user_available_balance);
...
if ($type == "DisplayCouponList") {
    ...
    $eSystemType = isset($_REQUEST['eSystem']) ? clean($_REQUEST['eSystem']) : ''; //'DeliverAll','General'

    $eType = isset($_REQUEST['eType']) ? clean($_REQUEST['eType']) : ''; //'Ride','Delivery','UberX','DeliverAll','General'

    //getAllAvailPromocodes($eType,"",$userType,$iMemberId);
    ...
    if ($userType == "Passenger") {
        ...
        $sqld = "SELECT ru.vCurrencyPassenger as vCurrency,ru.vLang,cu.vSymbol FROM register_user as ru LEFT JOIN currency as cu ON ru.vCurrencyPassenger = cu.vName WHERE iUserId = '" . $iMemberId . "'";
        ...
        $sqld = "SELECT rd.vCurrencyDriver as vCurrency,rd.vLang,cu.vSymbol FROM register_driver as rd LEFT JOIN currency as cu ON rd.vCurrencyDriver = cu.vName WHERE iDriverId = '" . $iMemberId . "'";
    ...
    //Added By HJ On 31-12-2018 For Get User Currency Ratio Start

    $userRatio = 1;

    $defaultCurrencyRatio = get_value('currency', 'vSymbol, Ratio', 'vName', $vCurrency);

    if (isset($defaultCurrencyRatio[0]['Ratio'])) {

        $userRatio = $defaultCurrencyRatio[0]['Ratio'];

    }

    //Added By HJ On 31-12-2018 For Get User Currency Ratio End

    $ssql = "";

    if ($eSystemType != "") {

        $ssql = " AND (eSystemType = '" . $eSystemType . "' OR eSystemType = 'General')";
    ...
    if ($eSystemType == "" && $eType == "") {

        $ssql = " AND eSystemType = 'General'";

    }

    $couponData = $Data_Filter = array();

    $tDescription = "tDescription_" . $vLang;

    $sql = "SELECT * from coupon WHERE eStatus = 'Active' AND iUsageLimit > iUsed" . $ssql;

    $couponData = $obj->MySQLSelect($sql);

    ## Filter Of Coupon Data ##

    $Data_Filter = $couponData;
    ...
    if (count($couponData) > 0) {
        ...
        for ($i = 0; $i < count($couponData); $i++) {
            ...
            if ($couponData[$i]['eType'] != "percentage") {

                $Data_Filter[$i]['fDiscount'] = $generalobj->setTwoDecimalPoint($couponData[$i]['fDiscount'] * $userRatio); //Added By HJ On 31-12-2018 Convert Default Currency Into User Currency

            }

            $tDescriptionArr = json_decode($couponData[$i]['tDescription'], true);



            $couponData[$i]['vCurrency'] = $vCurrency;

            $couponData[$i]['vSymbol'] = $vSymbol;



            $Data_Filter[$i]['vCurrency'] = $vCurrency;

            $Data_Filter[$i]['vSymbol'] = $vSymbol;



            $couponData[$i]['tDescription'] = $tDescriptionArr[$tDescription];

            $Data_Filter[$i]['tDescription'] = $tDescriptionArr[$tDescription];

            $isRemoveCouponFromList = "No";

            $vCouponCode = $couponData[$i]['vCouponCode'];

            $sql = "select iTripId from trips where vCouponCode = '" . $vCouponCode . "' and iActive = 'Finished' and $condfield ='" . $iMemberId . "'";

            $data_trip_coupon = $obj->MySQLSelect($sql);

            if (count($data_trip_coupon) > 0) {

                $isRemoveCouponFromList = "Yes";

            }

            $iUsageLimit = $couponData[$i]['iUsageLimit'];

            $iUsed = $couponData[$i]['iUsed'];
            ...
            $eValidityType = $couponData[$i]['eValidityType'];

            $dActiveDate = $couponData[$i]['dActiveDate'];

            $dExpiryDate = $couponData[$i]['dExpiryDate'];

            $curr_date = @date("Y-m-d");

            if ($eValidityType == "Defined") {

                if ($curr_date >= $dActiveDate && $curr_date <= $dExpiryDate) {
    ...
    $couponData = array_values($Data_Filter);
...
if ($type == "addMoneyUserWallet") {
    ...
    $UserCardData = get_value($tbl_name, 'vStripeCusId,vStripeToken,vBrainTreeToken,vPaymayaCustId,vXenditToken,vPaymayaToken,vFlutterWaveToken,' . $currencycode . ' as currencycode', $iUserId, $iMemberId);
...
if ($type == "sendRequestToDrivers") {
    ...
    if (($iHotelId == '' || $iHotelId < 1) && ($GeneralUserType == 'Hotel' || $GeneralUserType == "Kiosk")) {
    ...
    $UserDetail = get_value('register_user AS ru LEFT JOIN currency AS c ON c.vName=ru.vCurrencyPassenger', 'ru.vCurrencyPassenger,c.Ratio,c.vSymbol, ru.iGcmRegId,ru.vName,ru.vLastName,ru.vImgName,ru.vFbId,ru.vAvgRating,ru.vPhone,ru.vPhoneCode, ru.vLang', 'ru.iUserId', $passengerId);
    ...
    $result = $obj->MySQLSelect($sql_driver_status_chk);
    ...
    $Data_update_passenger['tVehicleTypeData'] = stripcslashes($orderDetails);
...
if ($type == "cancelTrip") {
    ...
    $tripCancelData = get_value('trips AS tr LEFT JOIN vehicle_type AS vt ON vt.iVehicleTypeId=tr.iVehicleTypeId', 'tr.vCouponCode,tr.vTripPaymentMode,tr.iUserId,tr.iFare,tr.vRideNo,tr.tStartDate,tr.tTripRequestDate,tr.tDriverArrivedDate,tr.eType,tr.ePaymentBy,tr.iOrganizationId,vt.fCancellationFare,vt.iCancellationTimeLimit,vt.iWaitingFeeTimeLimit,tr.ePoolRide,tr.tVehicleTypeFareData', 'iTripId', $iTripId);
...
if ($type == "GenerateTrip") {
    ...
    //if($ride_type != "Multi-Delivery" && $ride_type != "UberX"){
    ...
    if (checkDriverSubscriptionModule()) {
        ...
        if ($returnSubStatus == 1) {
            ...
            $returnArr['message'] = "LBL_PENDING_SUBSCRIPTION_TEXT";//in this 2 type use this insted of PENDING_SUBSCRIPTION, bc its checking from app side..so GP said put here
    ...
    // $isServiceLock = $generalobj->checkServiceLock($iCabRequestId, $iCabBookingId);
    ...
    if ($eStatus == "Completed") {
        ...
        if ($APP_TYPE != "UberX") {
            ...
            if ($iTripId != "" && $iTripId != 0) {
                ...
                if (($status_trip == "Active" || $status_trip == "On Going Trip") && ($trip_etype != "UberX" && $trip_etype != "Multi-Delivery") && $ride_type != "Multi-Delivery" && DELIVERALL == "No") {
    ...
    if ($eStatus == "Requesting" || (($eStatus == "Assign" || $eStatus == "Accepted") && $iCabBookingId != "" && $iCabRequestId == "")) {
        ...
        if ($iCabBookingId != "") {
            ...
            $sql_booking = "SELECT vSourceLatitude, vSourceLongitude,vSourceAddresss,vDestLatitude,vDestLongitude,tDestAddress,ePayType,iVehicleTypeId,iRentalPackageId,eType,iPackageTypeId,vReceiverName,vReceiverMobile,tPickUpIns,tDeliveryIns,tPackageDetails,fPickUpPrice,fNightPrice,iUserPetId,vCouponCode,iQty,vRideCountry,fTollPrice,vTollPriceCurrencyCode,eTollSkipped, vTimeZone,iUserAddressId,tUserComment,eFlatTrip,fFlatTripPrice,eWalletDebitAllow,eBookingFrom,iFare,iBaseFare,fPricePerMin,fPricePerKM,fCommision,fSurgePriceDiff,fTax1,fTax2,fTax1Percentage,fTax2Percentage,fOutStandingAmount,vDistance, vDuration, ePaymentByReceiver, fMinFareDiff, fTripGenerateFare,fDiscount,vDiscount,fWalletDebit,iTripReasonId,vReasonTitle,eTripReason,ePayWallet,eServiceLocation,tVehicleTypeData,tVehicleTypeFareData, vWorkLocation, vWorkLocationLatitude, vWorkLocationLongitude, eSelectWorkLocation, tTotalDuration, tTotalDistance,vBookSomeOneNumber,eBookForSomeOneElse,vBookSomeOneName,iUserId, tEstimatedCharge, tUserWalletBalance FROM cab_booking WHERE iCabBookingId='$iCabBookingId'";
        ...
        $Data_trips['tVehicleTypeData'] = $tVehicleTypeData;
...
if ($type == "ProcessEndTrip") {
    ...
    if ($eType == "Multi-Delivery") {
        ...
        $sql = "SELECT count(iTripDeliveryLocationId) AS TotalTripDelivery FROM `trips_delivery_locations` WHERE iActive IN ('On Going Trip','Active')  AND iTripId='" . $tripId . "'";
    ...
    $sql = "SELECT tStartDate,tEndDate,tDriverArrivedDate,iVehicleTypeId,tStartLat,tStartLong,eFareType,fRatio_" . $vCurrencyDriver . " as fRatioDriver, vTripPaymentMode,fPickUpPrice,fNightPrice, eType, fTollPrice,eFlatTrip,fFlatTripPrice,eHailTrip,iFare,fDiscount,iActive,eSignVerification,ePaymentCollect_Delivery,vDeliveryConfirmCode,iDriverId,iRunningTripDeliveryNo,iOrganizationId,ePaymentBy FROM trips WHERE iTripId='$tripId'";
    ...
    if ($eFareGenerated == "No") {
        ...
        //$sqlp = "SELECT co.vCountry,co.vCountryCode,co.eRoundingOffEnable FROM register_driver as rd LEFT JOIN country as co ON rd.vCountry = co.vCountryCode  WHERE rd.iDriverId = '" . $userId . "'";

        //$countryData = $obj->MySQLSelect($sqlp);

        //$vCountry = $countryData[0]['vCountryCode'];

        /* $sqlp = "SELECT cu.vName, cu.iCurrencyId, cu.eRoundingOffEnable FROM register_driver AS rd LEFT JOIN currency AS cu ON rd.vCurrencyDriver = cu.vName WHERE rd.iDriverId = '" . $driverId . "'";
        ...
        //$sqlp = "SELECT co.vCountry,co.vCountryCode,co.eRoundingOffEnable FROM register_user as ru LEFT JOIN country as co ON ru.vCountry = co.vCountryCode WHERE ru.iUserId = '" . $userId . "'";

        //$countryData = $obj->MySQLSelect($sqlp);

        //$vCountry = $countryData[0]['vCountryCode'];

        $sqlp = "SELECT cu.vName, cu.iCurrencyId, cu.eRoundingOffEnable, cu.Ratio FROM register_user AS ru LEFT JOIN currency AS cu ON ru.vCurrencyPassenger = cu.vName WHERE ru.iUserId = '" . $userId . "'";
        ...
        $sqld = "SELECT cu.vName, cu.iCurrencyId, cu.eRoundingOffEnable FROM register_driver AS rd LEFT JOIN currency AS cu ON rd.vCurrencyDriver = cu.vName WHERE rd.iDriverId = '" . $driverId . "'";
    ...
    if ($id > 0) {
        ...
        if ($eType == "Multi-Delivery") {
            ...
            if (count($TripDeliveryData) > 0) {
                ...
                $LBL_RECEIPENT_TXT = get_value('language_label', 'vValue', 'vLabel', 'LBL_RECEIPENT_TXT', " and vCode ='" . $vLang . "'", 'true');
                ...
                $returnArr['Running_Receipent_Detail'] = $LBL_RECEIPENT_TXT . " " . $iRunningTripDeliveryNo . " ( " . $vReceiverName . " )";
            ...
            $vDriverTripStatus = get_value('register_driver', 'vTripStatus', 'iDriverId', $trip_start_data_arr[0]['iDriverId'], '', 'true');
...
if ($type == "CollectPayment") {
    ...
    if ($COMMISION_DEDUCT_ENABLE == 'Yes') {
        ...
        if ($vTripPaymentMode == "Cash" && $isCollectCash == "" && ( $eType != "Multi-Delivery" || ($ePaymentCollect_Delivery == "No" && $eType == "Multi-Delivery"))) {
            ...
            if ($iFare >= $iBalance) {
                ...
                if ($walletamountofcreditcard > $iFare) {
    ...
    if ($id > 0) {
        ...
        if ($eBookingFrom == 'Kiosk') {
            ...
            $vEmailUser = get_value('register_user', 'vEmail', 'iUserId', $iUserId, '', 'true');
...
if ($type == "ScheduleARide") {
    ...
    if ($eType == "UberX") {
        ...
        if (isset($fareDetails['originalFareTotal']) && !empty($fareDetails['originalFareTotal'])) {
            ...
            $sqlv = "SELECT vt.iVehicleCategoryId,vc.iParentId,vt.eFareType,vt.ePickStatus,vt.eNightStatus,vt.fFixedFare, vt.iBaseFare, vt.iMinFare, vt.fMinHour,vc.ePriceType From vehicle_type as vt LEFT JOIN vehicle_category as vc ON  vc.iVehicleCategoryId = vt.iVehicleCategoryId WHERE vt.iVehicleTypeId = '" . $iVehicleTypeId . "'";
    ...
    $Data['tVehicleTypeData'] = stripcslashes($orderDetails);
...
if ($type == "checkBookings") {
    ...
    if (count($Data) > 0) {
                    ...
                    if (!empty($Data[$i]['tVehicleTypeFareData'])) {
                        ...
                        if ($ParentVehicleCategoryId == 0) {
                            ...
                            if (count($tVehicleTypeFareDataArr_fareArr) > 0) {
                                ...
                                $sql_parent_id = "SELECT (SELECT vcs.vCategory_" . $vLang . " FROM vehicle_category as vcs WHERE vcs.iVehicleCategoryId = vc.iParentId) as vCategory FROM vehicle_category as vc, vehicle_type as vt WHERE vc.iVehicleCategoryId = vt.iVehicleCategoryId AND vt.iVehicleTypeId = '" . $tVehicleTypeFareDataArr_fareArr[0]->id . "'";
        ...
        if (!empty($iVehicleCategoryIds_str_ufx)) {
            ...
            $sql_parent_cat_name = "SELECT vcs.vCategory_" . $vLang . " as vCategory, vcs.iVehicleCategoryId FROM vehicle_category as vcs WHERE vcs.iVehicleCategoryId IN ($iVehicleCategoryIds_str_ufx)";
...
if ($type == "cancelBooking") {
    ...
    $sql = "select cb.vBookingNo,cb.eType,concat(rd.vName,' ',rd.vLastName) as DriverName,concat(ru.vName,' ',ru.vLastName) as RiderName,ru.vEmail as vRiderMail,ru.vPhone as RiderPhone,ru.vPhoneCode as RiderPhoneCode,rd.vPhone as DriverPhone,rd.vCode as DriverPhoneCode,rd.vEmail as vDriverMail,rd.vLang as driverlang, ru.vLang as riderlang ,cb.vSourceAddresss,cb.tDestAddress,cb.dBooking_date,cb.vCancelReason,cb.dCancelDate from cab_booking cb
    ...
    if (count($Data_Car) > 0) {
        ...
        if (count($Data_Car) == 1 && $Data_Car[0]['eType'] == "UberX" && $TotalVehicles == 1 && $APP_TYPE == "Ride-Delivery-UberX") {
            ...
            for ($i = 0; $i < count($db_vehicle_new); $i++) {
                ...
                $db_vehicle_new[$i]['Enable_Hailtrip'] = ($enable_hail_flag == "Yes" && $APP_PAYMENT_MODE != "Card" && $ENABLE_HAIL_RIDES == "Yes") ? "Yes" : "No";
        ...
        if (count($db_vehicletype) > 0) {
            ...
            for ($i = 0; $i < count($db_vehicletype); $i++) {
                ...
                if (strtoupper(PACKAGE_TYPE) != "STANDARD") {
                    ...
                    $db_vehicletype[$i]['eRental'] = isRentalEnable($db_vehicletype[$i]['iVehicleTypeId']);
...
if ($type == "UpdateBookingStatus") {
    ...
    $sqldata = "SELECT eStatus,eType FROM `cab_booking` WHERE iCabBookingId ='" . $iCabBookingId . "'";
...
if ($type == "UpdateBookingDateRideDelivery") {
    ...
    if ($id > 0) {
        ...
        //added by JK for sms functionality on 18-7-2019
...
if ($type == "ChargePassengerOutstandingAmount") {
    ...
    $riderData = $obj->MySQLSelect("SELECT ru.vStripeCusId,ru.vStripeToken,ru.vCurrencyPassenger,ru.vBrainTreeToken,ru.vPaymayaCustId,ru.vFlutterWaveToken,ru.vPaymayaToken,ru.vXenditToken, ru.vCurrencyPassenger, ru.vLang, cu.vSymbol, cu.Ratio, co.eUnit FROM register_user as ru, currency as cu, country as co WHERE ru.iUserId='" . $iMemberId . "' AND cu.vName = ru.vCurrencyPassenger AND co.vCountryCode = ru.vCountry");
    ...
    if ($SYSTEM_PAYMENT_FLOW == 'Method-2' || $SYSTEM_PAYMENT_FLOW == 'Method-3') {
        ...
        $user_available_balance_wallet = $generalobj->get_user_available_balance($iMemberId, "Rider");
...
if ($type == "getServiceCategories") {
    ...
    if ($userId != "") {
        ...
        if (checkDonationModule()) {
            ...
            $sql2 = "SELECT iVehicleCategoryId, vLogo, eShowType,vBannerImage, iDisplayOrder,vCategory_" . $lang . " as vCategory, eCatType, eSubCatType, tBannerButtonText,iServiceId, eDeliveryType FROM vehicle_category WHERE eStatus='Active' AND iParentId='$parentId' " . $ssql . " ORDER BY iDisplayOrder,iVehicleCategoryId ASC";

        } else {

            $sql2 = "SELECT iVehicleCategoryId, vLogo, eShowType,vBannerImage, iDisplayOrder,vCategory_" . $lang . " as vCategory, eCatType, eSubCatType, tBannerButtonText,iServiceId, eDeliveryType FROM vehicle_category WHERE eStatus='Active' AND eCatType!='Donation' AND iParentId='$parentId' " . $ssql . " ORDER BY iDisplayOrder,iVehicleCategoryId ASC";
            ...
            if (count($Data) > 0) {
                ...
                for ($i = 0; $i < count($Data); $i++) {
                    ...
                    if ($Data[$i]['eCatType'] == "ServiceProvider" || $Data[$i]['eCatType'] == "MoreDelivery") {
                        ...
                        $sql3 = "SELECT iVehicleCategoryId, vLogo, eShowType,vBannerImage, vCategory_" . $lang . " as vCategory, eCatType, eSubCatType, tBannerButtonText, iServiceId, eDeliveryType FROM vehicle_category WHERE eStatus='Active' AND iParentId='" . $Data[$i]['iVehicleCategoryId'] . "' ORDER BY iDisplayOrder ASC";
...
if ($type == "getServiceCategoryDetails") {
    ...
    if ($iMemberId != "") {
        ...
        $sql2 = "SELECT iVehicleCategoryId,iParentId,eDetailPageView, iServiceId,vCategory_" . $lang . " as vCategory, tCategoryDesc_" . $lang . " as tCategoryDesc, tBannerButtonText, eCatType FROM vehicle_category WHERE iVehicleCategoryId='" . $iVehicleCategoryId . "'";
