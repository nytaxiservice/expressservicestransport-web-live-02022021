                        ...
                        <div class="driver-trip-table">
			      			<form  name="frmbooking" id="frmbooking" method="post" action="payment_request_a.php">
			        			...
			        			<table width="100%" border="0" cellpadding="0" cellspacing="0" id="dataTables-example" <?php echo $rtls; ?>>
									...
									<tbody>
										<? $fareTotal = $commTotal = $tipPriceTotal = $payTotal = $HotelCommisionTotal = $cardNo = 0; //added by SP if anyone request is of the card then only this btn shown for cardno only on 31-07-2019
											for($i=0;$i<count($db_dtrip);$i++) {
												...
												$fHotelCommision= $generalobj->trip_currency_payment($db_dtrip[$i]['fHotelCommision'],$db_dtrip[$i]['fRatio_'.$tripcurname]);
												$tipPrice = 0;
												if ($ENABLE_TIP_MODULE == "Yes") {
												$tipPrice = $generalobj->trip_currency_payment($db_dtrip[$i]['fTipPrice'],$db_dtrip[$i]['fRatio_'.$tripcurname]);
												}
												$payment=$fare-$Commission-$fHotelCommision;
												...
												<tr class="gradeA">
													...
													<? if(ENABLEHOTELPANEL == 'Yes' || ENABLEKIOSKPANEL == 'Yes') { ?>
													<td align="right"><?= $tripcursymbol;?><?=$fHotelCommision; $HotelCommisionTotal += $fHotelCommision; ?></td>
													<? } ?>
													<?php if ($ENABLE_TIP_MODULE == "Yes") { ?>
									...
									<tr class="last_row_record">
										...
										<? } ?>
										<?php if ($ENABLE_TIP_MODULE == "Yes") { ?><td class="last_record_row midddle_rw"><?= $tripcursymbol;?> <?php echo $generalobj->trip_currency_payment($tipPriceTotal); ?></td><?php } ?>
										<td class="last_record_row"> <?= $tripcursymbol;?><?php echo $generalobj->trip_currency_payment($payTotal); ?></td>
										<td></td>
										<td></td>
										<td></td>
									</tr>
