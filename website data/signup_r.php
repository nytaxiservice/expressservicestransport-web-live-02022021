    ...
    if ($valiedRecaptch) {
        if (isset($_POST['SUBMIT'])) {
            ...
            if ($checPhoneExist['status'] == 0) {
                ...
                header("Location:" . $tconfig["tsite_url"] . "sign-up-restaurant.php?error=1&var_msg=Phone number already exist.");
                exit;
            } else if ($checPhoneExist['status'] == 2) {
                header("Location:" . $tconfig["tsite_url"] . "sign-up-restaurant.php?error=1&var_msg=" . $langage_lbl['LBL_INVALID_MEMBER_USER_COUNTRY_EMAIL_TXT']);
                exit;
            }
        ...
        $_SESSION['postDetail'] = $_REQUEST;
        header("Location:" . $tconfig["tsite_url"] . "sign-up-restaurant.php?error=1&var_msg=" . $langage_lbl['LBL_CAPTCHA_MATCH_MSG']);
        exit;
    }
} else {
    $_SESSION['postDetail'] = $_REQUEST;
    header("Location:" . $tconfig["tsite_url"] . "sign-up-restaurant.php?error=1&var_msg=Please check reCAPTCHA box.");
    exit;
}
/* } else {
  $_SESSION['postDetail'] = $_REQUEST;
