...
Class General_admin {
        ...
        $_SESSION['sess_lang'] = "EN";
         if(isset($_SESSION['sess_iAdminUserId']) && !empty($_SESSION['sess_iAdminUserId'])){
            ...
            if($eStatus=='Deleted'){
                ...
                if(!$isAjax) $_SESSION['login_redirect_url'] = $_SERVER['REQUEST_URI']; //added by SP for redirection on admin after login on 15-7-2019, here put bc when page open from admin side and logout then open same link
            }
            if($checkadmin <=0){
    ...
    public function getStoreTripStates($OrderStatus = "", $tOrderRequestDate = "", $dDeliveryDate = "") {
        ...
        if ($OrderStatus != "") {
            ...
            if ($OrderStatus == "on going order") {
                $ssl .= " Where o.iStatusCode IN ('1','2','4','5')";
    ...
    public function getTotalEarns() {
        ...
        $sql = "SELECT SUM( `fCommision` ) AS total FROM trips WHERE iActive = 'Finished' AND eSystem = 'General' AND eCancelled = 'No' {$location_where} " . $cmp_ssql;
        $data = $obj->MySQLSelect($sql);
        $result = $data[0]['total'];
        return $result;
    }
    ...
    public function getTripDateStates($time) {
        ...
        $sql = "SELECT count(iTripId) as total FROM trips WHERE 1 = 1 AND eSystem = 'General' " . $ssl . $cmp_ssql . $location_where;
        $data = $obj->MySQLSelect($sql);
        return $data[0]['total'];
    }
    ...
    public function getAllCashCountbyDriverId($id, $ssql) {
        ...
        if ($id != "") {
            ...
            $sql = "SELECT SUM(fCommision) as totalAmount FROM trips AS tr WHERE vTripPaymentMode='Cash' AND eDriverPaymentStatus = 'Unsettelled' AND eSystem = 'General' AND iDriverId = '" . $id . "'" . $ssql;
            $data = $obj->MySQLSelect($sql);
    ...
    public function getAllCardCountbyDriverId($id, $ssql) {
        ...
        if ($id != "") {
            ...
            $sql = "SELECT SUM(fTripGenerateFare) as totalTripAmount,SUM(fCommision) as totalCommissionAmount,SUM(fOutStandingAmount) as totalOutstandingAmount FROM trips as tr WHERE eDriverPaymentStatus = 'Unsettelled' AND vTripPaymentMode='Card' AND eSystem = 'General' AND iDriverId = '" . $id . "'" . $ssql;
            $data = $obj->MySQLSelect($sql);
            //$total = ($data[0]['totalAmount'] != "")?$data[0]['totalAmount']:'0.00';
    ...
    public function getAllWalletCountbyDriverId($id, $ssql) {
        ...
        if ($id != "") {
            global $obj;
            $sql = "SELECT SUM(fWalletDebit) as totalAmount FROM trips AS tr WHERE vTripPaymentMode='Cash' AND eDriverPaymentStatus = 'Unsettelled' AND eSystem = 'General' AND iDriverId = '" . $id . "'" . $ssql;
            $data = $obj->MySQLSelect($sql);
            $total = ($data[0]['totalAmount'] != "") ? $data[0]['totalAmount'] : '0.00';
        }
        return number_format($total, 2);
    ...
    public function getAllPromocodeCountbyDriverId($id, $ssql) {
        ...
        if ($id != "") {
            global $obj;
            $sql = "SELECT SUM(fDiscount) as totalAmount FROM trips AS tr WHERE vTripPaymentMode='Cash' AND eDriverPaymentStatus = 'Unsettelled' AND eSystem = 'General' AND iDriverId = '" . $id . "'" . $ssql;
            $data = $obj->MySQLSelect($sql);
            $total = ($data[0]['totalAmount'] != "") ? $data[0]['totalAmount'] : '0.00';
        }
        return number_format($total, 2);
    ...
    public function getAllOutstandingAmountCountbyDriverId($id, $ssql) {
        ...
        if ($id != "") {
            global $obj;
            $sql = "SELECT SUM(fOutStandingAmount) as totalAmount FROM trips AS tr WHERE vTripPaymentMode='Cash' AND eDriverPaymentStatus = 'Unsettelled' AND eSystem = 'General' AND iDriverId = '" . $id . "'" . $ssql;
            $data = $obj->MySQLSelect($sql);
            $total = ($data[0]['totalAmount'] != "") ? $data[0]['totalAmount'] : '0.00';
        }
        return number_format($total, 2);
    ...
    public function getAllBookingAmountCountbyDriverId($id, $ssql) {
        ...
        if ($id != "") {
            global $obj;
            $sql = "SELECT SUM(tr.fHotelCommision) as totalHotelAmount FROM trips AS tr WHERE tr.vTripPaymentMode='Cash' AND tr.eDriverPaymentStatus = 'Unsettelled' AND tr.eSystem = 'General' AND tr.iDriverId = '" . $id . "'" . $ssql;
            $data = $obj->MySQLSelect($sql);
    ...
    public function getAllTipCountbyDriverId($id, $ssql) {
        ...
        if ($id != "") {
            global $obj;
            $sql = "SELECT SUM(fTipPrice) as totalAmount FROM trips AS tr WHERE eDriverPaymentStatus = 'Unsettelled' AND vTripPaymentMode='Card' AND tr.eSystem = 'General' AND iDriverId = '" . $id . "'" . $ssql;
            $data = $obj->MySQLSelect($sql);
            $total = ($data[0]['totalAmount'] != "") ? $data[0]['totalAmount'] : '0.00';
        }
        return number_format($total, 2);
