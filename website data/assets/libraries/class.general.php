    ...
    public function check_member_login() {
        ...
        if ($sess_iUserId == "" && basename($_SERVER['PHP_SELF']) != "login.php") {
            ...
            if (!empty($_SESSION['sess_user']) && $_SESSION['sess_user'] == 'rider' && !empty($_SESSION['sess_iUserId'])) {
                ...
                $sql = "SELECT COUNT(iDriverId) AS Total,eStatus FROM register_driver WHERE eStatus != 'Deleted' AND
    ...
    public function send_messages_user($type, $db_rec = '', $newsid = '', $maillanguage = '') {
        ...
        switch ($type) {
            ...
            case "USER_SEND_MESSAGE":
                ...
                $val_arr = array($db_rec['DRIVER_NAME'], $db_rec['PLATE_NUMBER'], $db_rec['PLATE_NUMBER'], $db_rec['BOOKING_DATE'], $db_rec['BOOKING_TIME'], $db_rec['BOOKING_NUMBER']);
                break;
  ...
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    public function clean($str) {
        ...
        if (!is_array($str)) {
            ...
            if ($inwebservice != "1") {
                ...
                $str = $obj->SqlEscapeString($str); //Commented By HJ On 06-09-2019 For Solved \r\n issue in Service type description As Per Discuss with KS sir
            }
            //$str = mysqli_real_escape_string($str);
        }
    ...
    public function InsertIntoUserWallet($iUserId, $eUserType, $iBalance, $eType, $iTripId, $eFor, $tDescription, $ePaymentStatus, $dDate) {
        ...
        if (($currencySymbol == "" || $currencySymbol == NULL) || ($currencyRatio == "" || $currencyRatio == NULL)) {
            $DefaultCurrencyData = get_value('currency', 'vSymbol,ratio', 'eDefault', 'Yes');
    ...
    public function get_benefit_amount($iTripId) {
        ...
        if ($count_rider > 0) {
            ...
            $sql = "SELECT iUserId,iRefUserId ,eRefType, CONCAT(vName,' ',vLastName) as tripusername from register_user where iUserId=" . $db_result[0]['iUserId'];
            $db_rider_user = $obj->MySQLSelect($sql);
            $count_rider_user = count($db_rider_user);
            ...
            if ($count_rider_user > 0) {
                ...
                if ($db_rider_user[0]['iRefUserId'] != 0) {
                    ...
                    $sql12 = "SELECT vEmail, CONCAT(vName,' ',vLastName) as username from register_user where iUserId='" . $db_rider_user[0]['iRefUserId'] . "'";
            ...
            $sql = "SELECT iRefUserId,iDriverId,eRefType, CONCAT(vName,' ',vLastName) as tripusername  from register_driver where iDriverId=" . $db_result[0]['iDriverId'];
            ...
            if ($count_driver_user > 0) {
                ...
                if ($db_driver_user[0]['iRefUserId'] != 0) {
                    ...
                    $sql12 = "SELECT vEmail, CONCAT(vName,' ',vLastName) as username from register_driver where iDriverId='" . $db_driver_user[0]['iRefUserId'] . "'";
...
############### Invoice Price For Web   ###################################################################
    ...
    public function getTripPriceDetailsForWeb($iTripId, $iMemberId = '', $eUserType = '', $eFrom = '', $isOrganization = '') {
        ...
        if ($currencycode == "" || isset($currencycode) && $currencycode == null) {
            $sql = "SELECT vName,vSymbol from currency WHERE eDefault = 'Yes'";
            $currencyData = $obj->MySQLSelect($sql);
        ...
        if ($eUserType == "Passenger") {
            ...
            if (isset($tripData[0]['fRoundingAmount']) && !empty($tripData[0]['fRoundingAmount']) && $tripData[0]['fRoundingAmount'] != 0 && $samecur == 1 && $currData[0]['eRoundingOffEnable'] == "Yes") {
                ...
                $tripFareDetailsArr[$i + 1]['eDisplaySeperator'] = "Yes";
                $i++;
                $tripFareDetailsArr[$i + 1][$languageLabelsArr['LBL_ROUNDING_NET_TOTAL_TXT']] = ($iActive != "Canceled") ? $generalobj->formatnum($roundingOffTotal_fare_amount, $currencycode) : $generalobj->formatnum($returnArr['fWaitingFees'], $currencycode);
            }
...
############################################################## getGeneralVarAll_Payment_Array ###############################################################################################################
    ...
    public function getGeneralVarAll_Payment_Array() {
        ...
        $generalConfigPaymentArr['SYSTEM_PAYMENT_FLOW'] = $SYSTEM_PAYMENT_FLOW;
...
############################################################## getGeneralVarAll_Payment_Array ###############################################################################################################
    ...
    //Added By HJ On 13-02-2019 For Set Dynamic Decimal Point In All APIs End
    public function checkServiceLock($iCabRequestId, $iCabBookingId, $isDeliverAll = false) {
        global $tconfig;
        $serviceId = empty($iCabRequestId) ? $iCabBookingId : $iCabRequestId;
        $fileName = $isDeliverAll == true ? "Order_" . $iCabRequestId : ( empty($iCabRequestId) ? "CabBooking_" . $iCabBookingId : "CabRequest_" . $iCabRequestId);
