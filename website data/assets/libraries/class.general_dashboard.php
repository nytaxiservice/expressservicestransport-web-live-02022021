<?php
	Class General_dashboard
	{
		...
		public function getTotalEarns()
		{
			...
			$sql = "SELECT SUM(`fCommision`) AS total FROM trips WHERE iActive = 'Finished' AND eSystem = 'General' AND eCancelled = 'No' ".$cmp_ssql.$location_where;
			$data = $obj->MySQLSelect($sql);
			$result = $data[0]['total'];
			return $result;
		}
		...
		public function getTripStatescount($tripStatus=NULL,$startDate="",$endDate="")
		{
			...
			if($tripStatus != "") {
				...
				$sql = "SELECT count(iTripId) tot_trip FROM trips WHERE 1 = 1 AND eSystem = 'General'".$cmp_ssql.$ssl.$dsql.$location_where;
				$data = $obj->MySQLSelect($sql);
			}
			return $data;
		}
		...
		public function getTripAmount($startDate,$endDate,$payment_method="")
		{
			...
			$sql = "SELECT iTripId, fTripGenerateFare, fDiscount,fWalletDebit, iFare, vTripPaymentMode FROM trips WHERE 1 = 1 AND eSystem = 'General' ".$cmp_ssql.$ssl.$dsql . $psql .$locations_where;
			$data = $obj->MySQLSelect($sql);
