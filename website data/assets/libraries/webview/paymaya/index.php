...
function paymaya_checkouts($itemName, $orderNo, $amount, $successUrl, $failedUrl, $paymayaAPIUrl, $paymaya_auth) {
    ...
    $data = '{"totalAmount": {"currency": "PHP","value": "' . $amount . '"},"items": [ { "name": "' . $itemName . '","quantity":"1","amount": { "value": "' . $amount . '"},"totalAmount": { "value": "' . $amount . '" } } ],"redirectUrl": {"success": "' . $successUrl . '","failure": "' . $failedUrl . '","cancel": "' . $failedUrl . '"},"requestReferenceNumber": "000141386713"}';
    $ch = curl_init();
...
$successUrl = $redirectUrl = $tconfig['tsite_url'] . $returnUrl . "?iUserId=" . $iUserId . "&iOrderId=" . $iOrderId . "&amount=" . $itemPrice . "&ccode=" . $currency . "&vOrderNo=" . $vOrderNo . $extraParameters . "&payStatus=succeeded&vPayMethod=" . $vPayMethod;
$failedUrl = $redirectUrl = $tconfig['tsite_url'] . $returnUrl . "?iUserId=" . $iUserId . "&iOrderId=" . $iOrderId . "&amount=" . $itemPrice . "&ccode=" . $currency . "&vOrderNo=" . $vOrderNo . $extraParameters . "&payStatus=" . $status . "&vPayMethod=" . $vPayMethod;
...
if (isset($data['redirectUrl']) && $data['redirectUrl'] != "") {
    ...
    $paymaya_url = $failedUrl;
}
?>
<script>window.location.replace("<?php echo $paymaya_url; ?>");
</script>