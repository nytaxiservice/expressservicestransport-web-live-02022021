...
class DBConnection {
    ...
    public function MySQLQueryPerform($table, $data, $action = 'insert', $parameters = '') {
        ...
        if ($action == 'insert') {
            ...
            $results = mysqli_query($conn, $query) or die("Query failed: " . mysqli_error($conn) . "<br>{$query}<br><br>");
    ...
    public function sql_query($sql = "") {
        ...
        $conn = $this->CONN;
        $results = mysqli_query($conn, $sql) or die(mysqli_error($conn) . "query fail <br>{$sql}<br><br>");
        if (!$results) {
            $message = "Query went bad!";
            $this->error($message);
            return false;
    ...
    public function ExecuteQuery($string) {
        ...
        $conn = $this->CONN;
        $result_data = mysqli_query($conn, $string) or die('Query failed!' . "<br>{$string}<br><br>");
