...
if($tablename !='' && $type1!='' && $type_id !=''){
	...
	if($type == 'rider'){
		...
		$type="organization";	
		header("Location:organization_login.php?action=".$type.""); exit;		
	}
	else{
		$type="driver";	
		header("Location:login_new.php?action=".$type.""); exit;	
...
if($_POST['submit']){
	...
	if($tablename !='' && $type_action!='' && $id !=''){
		if($newpassword == $vPassword){
			...
			if($type_action == 'organization'){				
				header("Location:organization_login.php?action=organization"); exit;
			}else if($type_action == 'organization'){
				header("Location:sign-in"); exit;
			}else{
